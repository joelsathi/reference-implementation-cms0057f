...
function createPayer(PayerFormData payload) returns error? {
    ...
    sql:ParameterizedQuery query = `INSERT INTO payers (id, name, email, address, state, fhir_server_url, app_client_id, app_client_secret, token_url, scopes, created_at, updated_at) 
        VALUES (${newPayerId}, ${payload.name}, ${payload.email}, ${payload.address}, ${payload.state}, ${payload.fhir_server_url}, ${payload.app_client_id}, ${payload.app_client_secret}, ${payload.token_url}, ${payload.scopes}, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`;
